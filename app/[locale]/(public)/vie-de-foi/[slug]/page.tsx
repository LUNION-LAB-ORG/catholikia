...
export async function generateMetadata({params}: Props): Promise<Metadata> {
	...
	const data = await obtenirTemoignageParIdAction(slug);
	const temoignage = data.data || null;

	return {
		title: `${temoignage ? temoignage.temoin : 'Effata'}`,
		description: temoignage?.about,
		openGraph: {
			title: `${temoignage ? temoignage.temoin : 'Effata'}`,
			description: temoignage?.about,
			url: `https://www.catholikia.com/vie-de-foi/${slug}`,
			siteName: "Catholikia",
			locale: "fr_FR",
			type: "website",
			images: [
				{
					url: temoignage?.photo || "https://www.catholikia.com/og-image.jpg",
					width: 800,
					height: 600,
					alt: temoignage?.temoin,
				}
			],
		},
	}
...
export default async function VieDeFoiDetailsPage({params}: Props) {
	...
	return (
		<Content className="flex flex-col items-center space-y-6 mt-[4rem]">
			<div>
				<div className="size-80 rounded-lg overflow-hidden mx-auto">
					<Image
