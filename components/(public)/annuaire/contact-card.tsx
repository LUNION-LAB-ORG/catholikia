      ...
      <CardContent className="p-6">
        <div className="text-center mb-4">
          <Avatar className="w-16 h-16 mx-auto mb-3">
            <AvatarImage src={contact.image} alt={contact.name} width={200} height={200} />
            <AvatarFallback className="bg-primary/10 text-primary">
              <User className="w-8 h-8" />
            </AvatarFallback>
          </Avatar>
          <h3 className="font-semibold text-foreground text-lg font-inter">{contact.name}</h3>
          <p className="text-sm  font-medium  text-[#0088FF] font-geist">{contact.title}</p>
          <p className="text-sm text-muted-foreground">{contact.diocese}</p>
          {/* <div className="flex items-center jus text-muted-foreground">
